...
class DiffieHellman:
  ...
  def __init__(self, seed, prime=None):
    ...
    #print(f"debug: {seed.hex()} | {prime} | {self.my_secret} | {self.iters}")
...
def bit_flip(x, bit_str):
  # print("bit-flip str:")
  flip_str = base64.b64decode(bit_str)
...
class Chal:
  ...
  def send(self, bit_str):
    alice = DiffieHellman(bit_flip(self.alice_seed, bit_str))
    self.alice = alice
